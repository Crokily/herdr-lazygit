...
| **Zoom** | `U`(候选 Z > U > X,分析见附录 A) | `files` / `commits, subCommits, reflogCommits`¹ / `stash` | `show-diff-pane.sh file/commit/stash` | 把当前选中对象放进宽 pane 看清楚:文件 diff / commit 全文 / stash 内容 |
| **Settings** | `;`(候选 `<c-s>` > O > `;` > `,`,分析见附录 A) | `global` | `open-settings-pane.sh` | 改插件自己的一切行为:AI 后端/模型/prompt、三动词键位、面板宽度 |

¹ reflog 面板的 context 名已实测确认为 `reflogCommits`(lazygit 0.63.0 启动时会校验
customCommands 的 context 名,非法名直接报错;临时 repo 里 reflog 标签页按 Zoom 键
正常触发,见附录 A)。
...
4. **reflog 面板 context 名 = `reflogCommits`**:lazygit 0.63.0 启动时会按 permitted
   列表校验 customCommands 的 context 名(伪造名会硬报错,阴性对照已验),临时 repo
   的 reflog 标签页按 KEY_ZOOM 实际触发成功——两路证据一致。
5. **disable 策略推论**:KEY_ZOOM 是面板级 custom,天然压过同面板与 universal 内置键,
   永不需要 `<disabled>`;只有 KEY_SETTINGS(global custom,会被任意面板的同键内置
   遮蔽)需要 `gen-config-layer.sh` 按需生成 `<disabled>`,且只 disable 非 universal
   段(global custom 本就压过 universal/global 内置)。默认键 `;` 全空闲,此段为空。
